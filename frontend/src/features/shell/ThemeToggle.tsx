 ...
 * Identical behavior to LiquiGuard's, only the localStorage key sniff
 * is updated to `walletsync.theme`.
 */
...
export function ThemeToggle() {
  ...
  // Snapshot the persisted theme from localStorage to guard against SSR
  // hydration mismatches. The inline boot script in layout.tsx already
  // applies the class to <html> before paint, so the post-hydration
  ...
  const persistedMode: ThemeMode = useSyncExternalStore(
    ...
    () => {
      ...
      try {
        const raw = window.localStorage.getItem("walletsync.theme");
